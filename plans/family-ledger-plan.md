...
>
> **状态：📋 需求设计阶段**（代码尚未开始） | 最后更新：2026-05-14
...
| Node.js | 18+ | 前端构建用（Vue 3） |
| Maven | 3.8+ | 后端构建 |
| 操作系统 | Windows 10+ / WSL2 | 开发 & 运行环境 |
...
| 🏪 超市 | 日用品集中采购（大卖场一站式） | ✅ | 与🛒日用区分：超市=批量采购，日用=零散购买 |
| 🛒 日用 | 垃圾袋、清洁用品等零散购买 | ✅ | 单品类、非集中采购 |
| 🎮 游戏 | 充值、购买 | ✅ | — |
| 🎬 娱乐 | 电影、门票、玩乐 | ✅ | — |
...
> **决策：** `amount` 采用正负号方案。正=收入，负=支出。不额外增加 `type` 字段，保持简洁。
...
### 3.5 自动交易表（auto_transaction）

> 用于管理周期性固定支出/收入模板，定时生成预填充交易草稿。

修改 `transaction` 表，增加 `project_id` 字段后，自动交易独立为以下表：
...
| **响应速度** | 列表查询 < 500ms，统计 < 2s（基于 3,000–30,000 笔数据量，本地 MySQL） |
...
系统提供 MySQL 直连入口供 Hermes Agent 调用，实现 **微信聊天记账**——你跟我说消费记录，我直接写数据库。

### 8.1 架构

```
微信 ←→ Hermes Agent (NLP分析)
                  ↓
           直接写MySQL（JDBC）
                  ↓
         family_ledger 数据库
...
- 不经过 Spring Boot API
- Hermes 持有一个只写/只读的 MySQL 账户（`hermes`），配置在 `~/.hermes/.env`
- 查询统计仍用 Web 前端或直接问 Hermes
...
LEDGER_DB_PORT=3306
LEDGER_DB_NAME=family_ledger
LEDGER_DB_USER=hermes
LEDGER_DB_PASS=***
```
...
GRANT SELECT, INSERT, UPDATE ON family_ledger.* TO 'hermes'@'127.0.0.1';
```

### 8.3 支持的口语指令

Hermes 解析用户的自然语言，自动识别以下信息并写入数据库：

| 我说 | Hermes 理解并写入 |
|:----|:-----------------|
| "今天充了50块钱电" | 交通→充电, -50, 汤圆爸爸 |
| "给汤圆买了包纸尿裤89块" | 母婴→用品, -89, 汤圆妈妈 |
| "晚上带娃出去吃了海底捞280" | 餐饮→在外就餐, -280, 汤圆爸爸 |
| "发工资了15k" | 收入→工资, +15000, 汤圆爸爸 |
| "5月12号超市买菜花了62" | 餐饮→超市买菜, -62, 指定日期 |
| "上周给妈转了2000" | 人情→孝敬长辈, -2000, 上周 |
| "这个月交通花了多少" | 查询统计，返回月汇总 |

**解析规则：**
- 省略日期 → 默认当天
- 省略归属人 → 问或默认当前对话人（default=汤圆爸爸, tangyuan-mama=汤圆妈妈）
- 金额含糊 → 问确认
- 分类匹配不上 → 列出相近分类让选
- 支持批处理：一次说多条用换行/逗号分隔
...
```
用户：今天充了50块钱电
Hermes：→ 解析：交通→充电, -50, 汤圆爸爸, 今天
        → 写入 DB
        → 回复：✅ 已记：🚗交通·充电 ¥50（今天）
        
用户：再记一个，超市买菜62
Hermes：→ 解析：餐饮→超市买菜, -62, 汤圆爸爸, 今天
        → 写入 DB
        → 回复：✅ 已记：🍜餐饮·超市买菜 ¥62（今天）
               📋 今日小计：2笔 / ¥112
```
...
|:----|:-----|
| "这个月花了多少" | 本月总支出+分类排名 |
| "交通这个月花了多少" | 交通大类及其细分明细 |
| "帮我看看上周母婴花了多少" | 按周统计+各细分 |
| "6月份我和汤圆妈妈各花了多少" | 每人汇总+占比 |
| "欧洲蜜月项目总共花了多少" | 项目总账+分类分布+每人 |
| "最近一笔大额支出是什么" | 当月最大单笔 |
...
| 入口 | 适合场景 |
|:----|:---------|
| **Hermes（微信）** | 随时记一笔、快速查账、不用开电脑 |
| **Web 前端** | 批量导入、改历史数据、看图表统计、管理分类 |

两条路径共用同一个 MySQL，数据实时同步，互不冲突。
